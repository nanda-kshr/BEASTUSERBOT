...
@events.register(events.NewMessage(outgoing=True, pattern=r'\.replyraid'))
async def _(event):
    global que
    await client.edit_message(event.message, que)
    if event.fwd_from:
        return
...
@events.register(events.NewMessage(outgoing=True, pattern=r'\.dreplyraid'))
async def _(event):
    global que
    if event.fwd_from:
        return
